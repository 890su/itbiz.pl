...
- `companyName` — необязательное объединённое поле для названия компании,
  организации или NIP;
- `contactName` — обязательно;
- `contact` — обязательное объединённое поле телефона или e-mail; endpoint
  нормализует значение в отдельную колонку `phone` либо `email`;
