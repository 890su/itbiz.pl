...
подтверждённого публичного телефона/email, немедленного production-уведомления
ответственному лицу и активного Google tag/Google Ads conversion tag. Все услуги
поэтому пока намеренно имеют `adEligible: false`.
...
Шаблон имени: `SRCH-{LANG}-{GEO}-CORE`, например `SRCH-PL-WAW-S-CORE` или
`SRCH-RU-SUB-PIA-CORE`. Благодаря этому любой язык или геозону можно остановить
...
### Географические ячейки

Нельзя одновременно таргетировать `Warszawa` целиком и её районы в структуре,
созданной для независимого включения районов: широкий target разрушит контроль.
Все 18 районов Варшавы распределяются ровно по одной ячейке:

| GEO       | Районы/населённые пункты                                                                  | Волна |
| --------- | ----------------------------------------------------------------------------------------- | ----- |
| `WAW-S`   | Mokotów, Ursynów, Wilanów                                                                 | 1     |
| `WAW-C`   | Śródmieście, Wola, Ochota, Żoliborz                                                       | 1     |
| `WAW-W`   | Włochy, Ursus, Bemowo                                                                     | 1     |
| `WAW-E`   | Praga-Północ, Praga-Południe, Targówek                                                    | 2     |
| `WAW-N`   | Bielany, Białołęka                                                                        | 2     |
| `WAW-SE`  | Wawer, Rembertów, Wesoła                                                                  | 2     |
| `SUB-PIA` | Piaseczno, Józefosław, Mysiadło, Nowa Iwiczna, Lesznowola, Magdalenka, Łazy               | 1     |
| `SUB-JAN` | Janki, Raszyn, Dawidy Bankowe, Falenty, Łady, Sękocin Stary, Sękocin Nowy                 | 1     |
| `SUB-KON` | Konstancin-Jeziorna, Bielawa                                                              | 2     |
| `SUB-W`   | Michałowice, Opacz-Kolonia, Reguły, Piastów, Pruszków; Nadarzyn только после проверки CPL | 2     |

Natolin, Kabaty, Imielin, Sadyba, Stegny, Służew, Powsin и Miasteczko Wilanów
не добавляются поверх соответствующего района отдельными target entities: это
создало бы пересечение. Они учитываются в отчётах по фактическому местоположению
и в текстах ключей, если есть спрос.

Перед импортом каждое название проверяется в Google Ads как доступная location
entity. Если малая местность недоступна, используется одна компактная radius-зона
для всей ячейки, а не несколько перекрывающихся радиусов. Между соседними
кампаниями ставятся взаимные location exclusions. `Presence — people in or
regularly in targeted locations` обязательно для каждой ячейки; режим
`presence or interest` запрещён.

Первая публикация в аккаунте: пять PL-ячеек `WAW-S`, `WAW-C`, `WAW-W`,
`SUB-PIA`, `SUB-JAN`. Их RU/EN/UK-клоны создаются paused и включаются отдельно
после языковой проверки. Остальные ячейки добавляются только после проверки
времени доезда и стоимости квалифицированной заявки.
...
1. создаются paused-кампании PL для `WAW-S`, `WAW-C`, `WAW-W`, `SUB-PIA` и
   `SUB-JAN`;
...
6. включение любой кампании выполняется отдельно по языку и GEO-ячейке, поэтому
   район или пригород можно остановить без влияния на остальные.
