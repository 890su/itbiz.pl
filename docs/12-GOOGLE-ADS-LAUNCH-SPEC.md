...
перелинковка проверены. D1 и Turnstile готовы принимать формы. Публичные контакты
подтверждены: `+48 573 012 321`, `info@itbiz.pl`. Пока нет немедленного
production-уведомления ответственному лицу. Google Ads tag и событие основной заявки реализованы через
...
1. [x] опубликованы подтверждённые телефон `+48 573 012 321` и email
       `info@itbiz.pl`;
...
Номер для Call asset подтверждён: `+48 573 012 321`. Сам asset добавляется после
подтверждения часов обработки звонков.
